...
const correctSchoolCalendar = () => ({
  ...
  transform(code, id) {
    ...
    const replacements = [
      ...
      ["style={entry.sessions.length ? subjectTextStyle : undefined}", "style={entry.sessions.length ? subjectTextStyle : (entry.isHoliday || entry.isExam ? { fontSize: '20px', fontWeight: 900, lineHeight: 1.1, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', margin: 0, padding: 0 } : undefined)}"],
